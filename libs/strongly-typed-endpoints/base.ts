...
type PATCHDATA<T> = { patch: { [K in keyof T]?: T[K] } }

type PATH_PARAMETERS<T> = { [K in keyof T]: string }[]
...
//     T extends QUERY<any> ? "Query not allowed in POST" : 
// T extends PATCHDATA<any> ? "Cannot use partial data for POST" : unknown> = T
